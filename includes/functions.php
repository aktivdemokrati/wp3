...
function responsive_breadcrumb_lists() {
	...
	// echo '<br>' . get_post_type_object($post_type);
	
	// echo body_class($class);
	$ad_the_title = get_the_title();
	// echo $ad_the_title;
	
	// single_post_title('Current post: ');

	if ( is_front_page()) {

		if ($showOnHome == 1) echo '<div class="breadcrumb-list"><a href="' . $homeLink . '">' . $text['home'] . '</a></div>';

/* AD2013 Removed WP-breadcrumb on page "Forum" */

	 } elseif ( $ad_the_title == 'Forum') {
		 
		 // Do not write the WP-breadcrumb
		...
		if ( is_home() ) {
			...
			is_single() && get_post_type() == 'forum' || 
			is_single() && get_post_type() == 'topic' || 
			!is_single() && get_post_type() == 'bp_group'  
			
			) {			
			
			echo 'Hem > '. bbp_breadcrumb();			 
    ...
    if (!function_exists('responsive_js')) {
        ...
        function responsive_js() {
            ...
            wp_enqueue_script('responsive-scripts', get_template_directory_uri() . '/js-dev/responsive-scripts.js', array('jquery'), '1.2.2', true);
			wp_enqueue_script('responsive-plugins', get_template_directory_uri() . '/js-dev/responsive-plugins.js', array('jquery'), '1.2.2', true);
        }
...
/* AD2013 Lade till widgetområden */		
		register_sidebar(array(
